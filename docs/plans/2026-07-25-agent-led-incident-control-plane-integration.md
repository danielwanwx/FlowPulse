...
| A versioned `IncidentProjection` is the only browser read model: canonical identity, connected-or-classified graph membership, impacted path/status, and action/gate/evidence revisions are explicit. | `IncidentCase` has state/run correlation; the P0 repository stores case/evidence/claims/events. The Node application has a separate demo projection. No Python incident graph projection exists. | Add strict projection/graph contracts, a tenant-scoped append-only projection store, canonical external incident identity, projection revision/sequence, and a classified-node allowlist. | `test_incident_workspace_contracts.py::test_projection_rejects_unclassified_or_noncanonical_nodes`; `test_live_incident_workspace_postgres.py::test_projection_revision_and_rls_are_monotonic`. |
...
| Backend generates a fixed-taxonomy `NextBestAction[]`: fixed title/CTA mapping, dynamic English summary/order/recommendation, evidence references, TTL/preconditions/permissions/version/invalidation. | P0 exposes manually submitted remediation proposals and dry-run receipts only. It has no action-card projection or taxonomy. | Define strict card enum/mapping and generate cards in a Temporal projection activity. Persist card/action version and invalidation reason. Cards may request a gate or review/track a dry run; they may not create an executable production action. | `test_next_best_actions.py::test_fixed_title_and_cta_mapping`; `test_next_best_actions.py::test_card_invalidates_on_evidence_gate_or_projection_revision`; `test_next_best_actions.py::test_no_card_bypasses_owner_gate`. |
...
| Versioned FastAPI projection, context, node-explanation, conversation, action, approval, and ordered SSE APIs are needed. Sequences and revisions are canonical; unavailable dependencies yield typed degraded/fail-closed results. | FastAPI offers `/healthz`, case intake/read, proposal, and dry-run only. It has no workspace projection or SSE endpoint. | Add versioned, authenticated routes backed solely by Temporal-authoritative projections, strict request/response schemas, event envelope, `Last-Event-ID`/`after` ordering, and typed degraded states. | `test_incident_workspace_api.py::test_route_schemas_and_tenant_scoping`; `test_incident_workspace_sse.py::test_resume_is_strictly_after_checkpoint`; `test_live_incident_workspace_http.py::test_intake_to_projection_to_sse_to_owner_gate`. |
| Cross-worktree E2E must use real Compose Temporal/Postgres/MinIO and expose workflow/run IDs plus evidence refs. Credential-free test/demo providers are visibly labeled; a separately opt-in local/open-source smoke is allowed without production credentials. | Compose already runs real Temporal/Postgres/MinIO, worker, API, and local fixture auth. P0 live tests prove its ownership path, but no Incident Workspace contract or cross-worktree evidence exists. | Extend Compose/migrations/worker for the workspace workflow and create a reproducible backend contract fixture. Publish only the API necessary for the UI; retain internal-only Temporal/authz and worker-only storage credentials. Record a frontend handoff that uses the API, not browser mocks. | `test_live_incident_workspace_http.py`; `test_live_incident_workspace_temporal.py`; `test_live_incident_workspace_postgres.py`; `test_live_incident_workspace_minio.py`; `test_live_frontend_contract_api.py`; opt-in `test_live_provider_smoke.py`. |
...
**Records.** Every workspace command carries the authenticated tenant, canonical incident/case/run IDs, workflow version, projection revision, and correlation ID. Activities append projection events/tool-call records and link evidence IDs and content hashes; provider calls record only safe metadata, prompt/context hashes, output schema version, latency/token counters, and truth/degraded labels. Raw telemetry, prompts, model payloads, credentials, and chain-of-thought are excluded from browser and SSE projections.
...
Do not mutate `flowpulse.diagnosis.v1` or its frozen replay behavior. Keep `flowpulse.diagnosis.v2` available for P0 callers. New Incident Workspace intake starts a separate workflow type, proposed as `flowpulse.incident-workspace.v1`, registered alongside the compatibility workflows in `control_plane/flowpulse_cp/temporal_runtime.py`. A new workflow type avoids changing recorded v1/v2 histories; later semantic evolution of this type must use Temporal patch/version markers and archived-history replay before release.
...
`IncidentProjection` is an immutable, versioned read model. Its canonical fields are:

```text
schema_version, tenant_id, incident_id, case_id, workflow_id, workflow_run_id,
projection_revision, sequence, lifecycle_state, status, generated_at,
...
| --- | --- |
| `POST /v1/incidents` | Starts `flowpulse.incident-workspace.v1`; returns `IncidentProjection` plus real `case_id`, `workflow_id`, and `workflow_run_id`. |
...
| `POST /v1/incidents/{case_id}/node-explanations` | Accepts only `component_id`, expected `projection_revision`, and an idempotency key; submits the Temporal start-or-reuse update. |
...
| `POST /v1/incidents/{case_id}/conversations` | Explicit user submit only; carries a bounded user intent, expected revision, conversation ID, and idempotency key. |
| `GET /v1/incidents/{case_id}/next-best-actions` | Returns server-generated fixed-taxonomy cards. |
| `POST /v1/incidents/{case_id}/gates/1` and `DELETE /v1/incidents/{case_id}/gates/1/{grant_id}` | Submit Gate 1 grant/revoke commands; no FastAPI-side authorization state mutation. |
| `POST /v1/incidents/{case_id}/proposals` and `POST /v1/incidents/{case_id}/proposals/{proposal_id}/dry-run` | Workspace aliases that submit to the existing exact Owner Gate command path; no direct approval/action record write. |
| `GET /v1/incidents/{case_id}/events` | SSE projection stream with strict checkpoint/reconnect semantics. |

Every authenticated API lookup, including events, explanation, cards, proposal, approval, and evidence lookup, is tenant-scoped before returning a 404/403. Route bodies do not self-assert tenant, subject, role, provider mode, evidence, authority, gate state, or action success.
...
Add a Gate 1 record with the exact scope `(tenant, case, workflow_run_id, component set, environment, capability set, data class, projection/evidence/capability-registry revisions, issued_at, expires_at, actor, status)`. It is created/revoked/expired through workspace Temporal updates and projected by activities. Before invoking any fresh read, the adapter validates authenticated subject permissions, active status, TTL, scope, and every bound revision. Revoke, expiry, source freshness change, projected component change, or relevant evidence/action revision invalidates the old grant and produces a bounded event/card update.

Gate 2 stays the existing Owner Gate: exact proposal contract hash, case/run/revision, owner identity assertion, approval TTL, ordered targets, canary maximum, preconditions/current witness, idempotency binding, and dry-run-only action receipt. The workspace workflow invokes the already-protected Owner Gate activity; it must not create a parallel approval or action writer.

`NextBestAction` is a strict server-generated projection. It permits no caller-provided title, CTA, recommendation, scope, evidence, or gate state. It has an ID, fixed taxonomy ID/title/CTA label, dynamic English summary, display order, one-or-zero `recommended`, evidence refs, required permission/gate, TTL, preconditions, version bundle, invalidation rules/status, and dry-run status. The only taxonomy mappings are:
...
At most three cards are returned and at most one is recommended. Card generation must preserve the current-proof/Knowledge-prior distinction, tenant/subject ACL, independent critic/verifier results, durable contradiction/coverage ledger, and no-production-write rule. A card that needs proof/gate/owner approval reports that need; it cannot imply that it has been granted.
...
1. `003_incident_workspace_projection.sql`: external incident identity, `incident_projections`, graph node/edge snapshots, immutable `incident_projection_events`, node-explanation selections/results, and a monotonic per-case/run sequence/revision constraint.
2. `004_incident_workspace_control_records.sql`: prompt/context/version bundles, conversation turns, provider invocations, capability registry snapshots/calls, Gate 1 grants, `next_best_actions`, and all supporting tenant/case/run composite foreign keys.

Every new tenant-bearing table has `ENABLE ROW LEVEL SECURITY`, `FORCE ROW LEVEL SECURITY`, tenant policies, and subject ACL where evidence/context requires it. Child tables use composite tenant/case/run/revision FKs or equivalent constraints so a record cannot be rebound across a tenant, case, run, or revision. Ledger/event rows are INSERT-only with no update/delete grants; projection replacement is limited to the Temporal activity projection transaction and must advance revision/sequence rather than `ON CONFLICT DO NOTHING` an updated state.
...
**Tests first.** Add `control_plane/tests/test_incident_workspace_docs_contract.py` to assert the three documents state the same authority boundary, declare the workspace workflow version, label test/demo provider behavior, and link the read-only frontend spec revision. Add a contract fixture under `control_plane/tests/fixtures/incident_workspace_v1/` containing only safe request/response/event examples that the frontend owner can consume.
...
**Acceptance.** Docs call FlowPulse a model-agnostic company-multiplayer agent OS with incident response first; they preserve Temporal, evidence, and authority constraints. The contract names API schema versions, workflow/run IDs, evidence refs, provider labels, and the no-browser-mock expectation. Existing P0 documentation/commands remain correct.
...
**Tests first.** Create `test_incident_workspace_contracts.py` and `test_live_incident_workspace_postgres.py` before models/migrations. Cover unknown field and primitive coercion rejection; canonical IDs; connected-or-classified graph membership; revision/sequence monotonicity; fixed CTA mapping; composite tenant/case/run/revision rejection; RLS; no update/delete of immutable records; and upgrade from preserved 001/002 data.
...
**Acceptance.** The database accepts only tenant-scoped append-only workspace records bound to a real case/run/revision, advances a projection only through the activity projection path, and fails closed for cross-tenant or stale references. The 001 hash/baseline and 002 authorization consumption behavior remain unchanged.
...
**Tests first.** Add `test_incident_workspace_api.py` and `test_incident_workspace_sse.py` for exact `/v1/incidents` schemas, tenant lookup, projection revision/sequence, terminal/degraded states, `Last-Event-ID` precedence/validation, reconnect strictly-after behavior, and no tool/provider invocation on GET/SSE.
...
**Acceptance.** FastAPI only reads tenant-scoped temporal-authoritative projections or submits typed commands. An event emitted by an activity is persisted before it becomes visible. A stale projection revision, missing event checkpoint, cross-tenant case, or unavailable projection fails closed/degraded without reconstructing state in the browser.
...
**Tests first.** Add `test_workspace_workflow.py`, `test_node_explanations.py`, and `test_workspace_replay.py`. Prove intake uses the new workflow type; an update arriving before the first workflow task is safely ordered; concurrent same-key node explanation requests generate/reuse one result; a different revision makes a distinct result; toast/focus has no workflow/API side effect; and pre-Gate-1 context has no fresh capability call/current claim.
...
**Acceptance.** The worker registers the new workflow and its activities while retaining frozen v1/v2 registrations. New raw histories are archived with producer SHA, workflow/run IDs, event counts, and SHA-256, then replayed by the exact current workflow. Existing `test_live_temporal_replay_compat.py` continues to replay parent v1 histories without current-class history generation.
...
**Tests first.** Add `test_gate1_capabilities.py`, `test_next_best_actions.py`, and `test_incident_workspace_safety.py`. Test the whole invalidation matrix (TTL, revoke, projection/evidence/registry revision change, wrong tenant/component/capability/data class), card maximum/recommendation/taxonomy, current-proof versus KB-prior prevention, critic/verifier independence, and no direct FastAPI approval/action write. Re-run existing exact Owner Gate/negative tests without weakening any assertion.
...
**Acceptance.** Gate 1 permits only the precise fresh-read call it authorizes. Gate 2/Owner Gate sees the exact existing proposal/approval/witness state and only produces an `APPROVED_READ_ONLY`/dry-run receipt after revalidation. All action cards remain recommendations; none performs an external mutation.
...
**Tests first.** Add `test_live_incident_workspace_http.py`, `test_live_incident_workspace_temporal.py`, `test_live_incident_workspace_minio.py`, `test_live_frontend_contract_api.py`, and `test_workspace_evaluation.py`. The contract test performs real API intake → authoritative run/projection → node explanation → explicit Gate 1 → bounded read (when a test adapter is bound) → card/proposal → existing Owner Gate dry run → ordered SSE resume. It asserts API response and events expose the actual Temporal workflow/run ID and evidence refs, and verifies corresponding Postgres/MinIO records. It never stubs browser fetches or claims the deterministic provider is live.
...
**Evidence paths/records.** The live tests must print or save a safe run manifest under `/tmp/flowpulse-incident-workspace-evidence/<run-id>.json` containing case/workflow/run IDs, projection/event sequence ranges, evidence IDs/content hashes, provider truth label, and MinIO object keys—not secrets or raw evidence. The durable equivalents are the tenant-scoped `incident_projections`, `incident_projection_events`, workspace control records, existing `case_events`/verification records, and tenant-prefixed MinIO artifacts. A test cleanup removes only its own `/tmp/flowpulse-incident-workspace-evidence` directory.
...
1. Start the isolated backend Compose stack with the explicit local fixture owner token above.
2. Point the frontend's API base URL to `http://127.0.0.1:8090` and authenticate using the configured fixture bearer only in local/test mode.
3. Create/read a real workspace incident, capture the returned `case_id`, `workflow_id`, `workflow_run_id`, `projection_revision`, and evidence refs, then use them in graph/card/SSE assertions.
4. Assert a node click yields the same durable explanation ID for the same selection key, a toast/focus yields no API call, pre-Gate-1 output states no fresh read, and provider labels accurately show test/demo/degraded/live state.
5. Assert gate/card/owner transitions appear as ordered SSE/API projection events. It must not intercept or fixture browser fetch responses for these assertions.
...
Before the implementation change is offered for review, record the exact backend SHA, frontend input SHA, migration checksums, workflow type/version, deterministic test count, live test count, skipped opt-in provider smoke status, and test manifest paths. Require:
...
- real Compose API/Temporal/Postgres/MinIO evidence with no host exposure regression for Temporal/authz and no browser mock in the backend/frontend contract proof;
