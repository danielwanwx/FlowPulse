...
**Cross-branch contract-freeze record.** The immutable planning input is `backend_baseline_sha=45d331e1523d9643a0321c8f65ce6a26b949660c` and its backend control-plane Git tree OID is `control_plane_git_tree_oid=cf8e013fea8e5ffc9d61758245536e4fb978cc8b`. The frontend input is `frontend_spec_sha=4adb8f5c0c49382e894732318530c83d9a0b5611`. No OpenAPI/schema example artifact exists at this plan-only checkpoint. Before a frontend product integration consumes any new field, the backend must commit the generated OpenAPI document and safe schema/example bundle, record the producing backend implementation SHA plus SHA-256 in this plan/contract, and the frontend owner must copy that exact record into the frontend integration plan. Until both records exist and match, implementation may proceed only through documentation, contracts, models, and tests—not product UI integration.
