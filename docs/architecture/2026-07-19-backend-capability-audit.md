...
2. A **bounded local-development compatibility path** composes frozen evidence, evaluator and diagnosis gates, checked-in policy artifacts, freshness, failure locks, an atomic Owner Gate, one execution-attempt claim, verification, and backtest. Its non-exported local decision and Owner Gate closure is private to `src/server.mjs`; it is not a production or Temporal lifecycle authority. Tests exercise concurrency and adversarial collisions. A QA record proves a real local deterministic Astronomy Shop recovery. No successful end-to-end GPT-5.6 recovery is currently proven.
...
The strongest foundations are the append-only SQLite ledger, the non-exported local decision boundary, bounded evidence freezing, strict IncidentProjection v1, deterministic replay, and tested connector sidecar contracts. The biggest integration gaps are:
...
| `src/server.mjs` | Unique HTTP/static composition root; owns ledger, sources, non-exported local decision closure, private Owner Gate/execution claim, projection and APIs | environment, HTTP requests, checked-in artifacts, local sources | HTTP/SSE responses and canonical ledger events | process entry point | SQLite, optional provider/observability/local adapter operations | local compatibility composition root; never Temporal lifecycle authority | `test/server.test.mjs` plus nearly all focused backend suites |
...
Behavioral and export-graph tests prove there is no public provider factory, receipt issuer, snapshot registration store, approval callback, capability token, or runtime execution method. `DevelopmentRuntime` stops before authority. The local compatibility decision, approval claim, and adapter invocation remain non-exported in `src/server.mjs` and are reachable only through bounded server routes.
...
9. Investigation single-flight and private receipt-context binding are process-local. A restart removes in-memory snapshot/binding context and should make unresolved authority non-actionable. This is acceptable for the local POC but not a durable distributed control plane.
