...
| 0:58–1:22 | In the Agent Team drawer, select **View Diagnosis** after it appears. Follow the Orchestrator handoff into Investigator. | The complete Diagnosis workspace opens on the faulted node and its bounded upstream/downstream subgraph. Explicit routing, active-agent work, evidence and safe messages update without pausing the autonomous loop. | “Orchestrator assigns the right work to the right specialist. Investigator follows logs, metrics, traces, deployment changes, and topology evidence instead of guessing from a single alert.” |
| 1:22–1:42 | Highlight the root-cause conclusion and the downstream Kafka signal, then the Evaluator verdict. | The endpoint mismatch is marked as the initiating cause. Kafka lag is rejected as a later symptom. Evaluator checks citations and causal order. | “The endpoint mismatch is the initiating cause; Kafka lag appears later. Evaluator independently challenges that conclusion before any repair can proceed.” |
| 1:42–2:12 | Select **Open Recovery Console** when the evaluated plan appears. | Recovery Console shows the plan, risk, authority, owning actor, safe execution activity, repair events and live recovery signals. The local endpoint repair runs without a human click. | “Because this repair is local, reversible, and policy-approved, FlowPulse executes it autonomously while exposing every action and handoff. Higher-risk actions would stop at a human gate.” |
| 2:12–2:34 | Return briefly to the recovering Live graph, then select **Compare Recovery** after it unlocks. | Payment and checkout recover, downstream lag converges, pulses return, and Compare aligns verified before/after values. | “FlowPulse does not stop when a command succeeds. It verifies that the root condition disappeared, direct symptoms cleared, and downstream behavior recovered.” |
