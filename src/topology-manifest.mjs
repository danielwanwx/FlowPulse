...
export const TOPOLOGY_MANIFEST_SCHEMA_VERSION = "flowpulse.topology-manifest.v1";
...
export function validateAstronomyIncidentSubgraph(manifest, bundle) {
  ...
  return deepFreeze({
    ...
    edges: edgeIds.map((id) => {
      ...
      return { id, from, to, relation: baseEdgeIds.has(id) ? "observed_dependency" : "incident_evidence" };
    })
  });
}

function validateTopologyManifest(value) {
  const rootFields = ["schema_version", "fixture_id", "source_system", "source_version", "evidence_mode", "execution_mode", "source_health", "captured_at", "derivation", "nodes", "edges", "content_sha256"];
  ...
  if (!Array.isArray(value.nodes) || !Array.isArray(value.edges) || value.nodes.length !== TOPOLOGY_MANIFEST_LIMITS.nodes || value.edges.length !== TOPOLOGY_MANIFEST_LIMITS.edges) fail("topology_manifest_count_invalid");
